...
/** 레스토랑 로고 이미지 업로드 → public URL 반환 (E-1 upload 패턴 재사용) */
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const path = `${LOGO_PREFIX}/${restaurantId}/${crypto.randomUUID()}.${safeExt}`;
    ...
    if (error) {
      console.error('[restaurant logo upload]', error);
    ...
    console.error('[restaurant logo upload]', err);
